  ...
  try {
    ...
    const projects = projectInfos.map(info => {
      ...
      const teamMembers = projectTeamMembers
        ...
        .map(member => ({
          userName: member.user.userName, // 팀원 이름
          userKey: member.user.userKey,   // 팀원 Key
          userId: member.user.userId,     // 팀원 ID
        }));

      return {
        userName: info.user.userName, // 요청한 사용자의 이름
        userKey: info.user.userKey,   // 요청한 사용자의 Key
        userId: info.user.userId,     // 요청한 사용자의 ID
        ...info.project,
        tasks: relatedTasks, // 해당 프로젝트에 연결된 Task 데이터 추가
        teamMembers, // 해당 프로젝트에 연결된 팀원 정보 추가
      };
    });
